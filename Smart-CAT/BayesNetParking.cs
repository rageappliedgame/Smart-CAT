    ...
    public static partial class BayesNet
    {
                    ...
                    (int[][], int[][][]) OutputLabels)
        {
            ...
            for (int x = 0; x < CompetencyModel.Item1.Length; x++)
            {
                ...
                {
                    String arff_c = Path.Combine(MyData, "CA", CompetencyModel.Item1[x] + ".arff");
                ...
                for (int y = 0; y < CompetencyModel.Item2[x].Length; y++)
                {
                    ...
                    {
                        String arff_f = Path.Combine(MyData, "CA", CompetencyModel.Item1[x], CompetencyModel.Item2[x][y] + ".arff");
            ...
            string Facet)
        {
            ...
            using (StreamWriter file = new StreamWriter(Path.Combine(MyData, Competency, Facet + "_GaussianMixtureModelAccord.txt")))
            {
                int noCl = 0;
            ...
            String Facet)
        {
            ...
            //! Save BayesNet results in .txt file
            using (StreamWriter file = new StreamWriter(Path.Combine(MyData, Competency, Facet + "_MeanShiftAccord.txt")))
            {
                for (int x = 0; x < GameLogs.Length; x++)
                {
                    file.Write("Instance {0}: ", x);
            ...
            (int[][], int[][][]) ExternalDataCF)
        {
            ...
            for (int x = 0; x < CompetencyModel.Item1.Length; x++)
            {
                ...
                {
                    String arff_c = Path.Combine(MyData, "CA", "Randomized", CompetencyModel.Item1[x] + ".arff");
                ...
                for (int y = 0; y < CompetencyModel.Item2[x].Length; y++)
                {
                    ...
                    {
                        String arff_f = Path.Combine(MyData, "CA", "Randomized", CompetencyModel.Item1[x], CompetencyModel.Item2[x][y] + ".arff");
